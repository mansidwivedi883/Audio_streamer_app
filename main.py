...
def message(data):
    ...
    if audio_data:
        ...
        audio_filename = f"audio_{session.get('name')}_{int(time.time())}.wav"
    ...
    print(f"{session.get('name')} said: {data.get('message')}")
    if content["audio"]:
        print(f"{session.get('name')} sent an audio file: {content['audio']}")
